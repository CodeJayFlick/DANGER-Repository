...
# Date of Revision: November 27th, 2024
...
from reformer_pytorch.reversible import ReversibleSequence, ReversibleBlock
...
train_code, test_code, train_labels, test_labels = train_test_split(
    df["code_sample"], df['msg_type'], test_size = 0.10, random_state = 42)
...
class Tokenizer:
    ...
    def text_to_sequences(self, texts):
        ...
        text_sequences = [
            [self.vocab.get(char, 0) for char in text] for text in texts
        ]
        return text_sequences
    ...
    def padding(self, text_sequences):
        ...
        for seq in text_sequences:
            if len(seq) > self.max_tokens:
                padded_sequences.append(seq[:self.max_tokens])
            else:
                padded_sequences.append(seq + [0] * (self.max_tokens - len(seq)))
...
class Reformer_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.torch(self.sequences[idx], self.labels[idx])
...
class LocalLSHAttention(nn.Module):
    ...
    def _apply_buckets(self, x, buckets):
        batch_size, seq_len, dim = x.shape
        output = torch.zeros_like(x)

        for hash_idx in range(self.n_hashes):
            round_buckets = buckets[:, hash_idx, :]
            unique_buckets = torch.unique(round_buckets)

            for bucket_id in unique_buckets:
                bucket_mask = torch.eq(round_buckets, bucket_id).unsqueeze(-1)
                if bucket_mask.sum() == 0:
                    continue

                bucket_x = x * bucket_mask
                attention_weights = torch.bmm(bucket_x, bucket_x.transpose(1, 2)) / (dim ** 0.5)
                attention_weights = torch.softmax(attention_weights, dim = -1)
                attention_weights = torch.dropout(attention_weights)

                bucket_output = torch.mm(attention_weights, bucket_x)
                output += bucket_output * bucket_mask
    ...
    def forward(self, x, input_mask = True, input_attn_mask = True, pos_emb = None, **kwargs):
        ...
        buckets = self.hash_vectors(x, num_buckets)

        if input_mask is not None:
            x = x * input_mask.unsqueeze(-1)

        output = self._apply_buckets(x, buckets)
...
"""
        ...
        for hash_idx in range(self.n_hashes):
            round_buckets = buckets[:, hash_idx, :]
            for bucket_id in torch.unique(round_buckets):
                bucket_mask = torch.eq(round_buckets, bucket_id)
                if bucket_mask.sum() == 0:
                    continue
...
class Reformer(torch.nn):
    def __init__(self,
        ...
        self.bucket_size = bucket_size
        self.num_hashes = num_hashes
        self.num_tokens = nn.Embedding(num_hashes, dim)
        self.ff_chunks = ff_chunks
        self.ff_mult = ff_mult
        self.ff_dropout = ff_dropout
        self.post_att_dropout = post_att_dropout
        ...
        reversible_blocks = [
            ReversibleBlock(
                f = LSHSelfAttention(dim, heads),
                g = FeedForward(dim, dropout = 0.1)
            )
            for _ in range(depth)
        ]

        self.reversible_layers = ReversibleSequence(reversible_blocks)
        self.norm = nn.LayerNorm(dim)
        self.classifier = nn.Linear(dim, num_classes)

    def forward(self, x):
        x = self.token_emb(x) + self.pos_emb(torch.arange(x.size(1), device = x.device))
        x, _ = self.reversible_layers((x, x))
        x = self.norm(x[0])
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for inputs, labels in self.test_loader:
                inputs, labels = inputs.to(self.device).long(), labels.to(self.device)
