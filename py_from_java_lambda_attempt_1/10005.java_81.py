...
def format_exception(exception):
    ...
    return '\n'.join(stack_trace)
```
Note that I've used Python's built-in `collections` module to create an empty list (`self.events`) instead of using Java-style syntax. Additionally, I've replaced the `@Override` annotations with Python's method definition syntax.

Also, please note that this is a translation and not a direct copy-paste from Java to Python. Some changes were made to adapt the code to Python's syntax and semantics.