...
if __name__ == "__main__":
    main()
```
Note that I had to make some assumptions about the `GhidraSwinglessTimer` and `GhidraSwingTimer` classes, as they are not defined in the original Java code. In Python, we don't have a direct equivalent of Java's Swing framework, so I used simple threading and time.sleep() calls instead.

Also, I replaced the `System.out.println()` statements with regular print statements using f-strings (Python 3.6+).