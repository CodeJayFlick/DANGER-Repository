...
This Python code defines a `PriorityQueue` class that maintains a list of objects in priority order. The priority is just an integer value. Objects can be added to the queue using the `add()` method, and the object with the lowest or highest priority number can be retrieved using the `get_first()` or `get_last()` methods respectively.