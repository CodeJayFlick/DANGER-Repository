...
Please note that this is not the actual translation of Java code into Python. The provided Java code has some parts which are specific to a certain framework or library (like `GDirectedGraph`, `Accumulator` and etc.), so it's hard to translate them directly into Python without knowing what those classes represent in your context.

Also, note that I have not implemented the missing methods (`get_strongly_connected_components` and `create_sub_graph`) as they are specific to a certain framework or library.