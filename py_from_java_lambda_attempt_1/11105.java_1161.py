...
Please note that Python does not have direct equivalent of Java's `RandomAccessFile` and `ByteArrayOutputStream`. We are using built-in file operations (`seek`, `readinto`) for the former, and a combination of byte arrays and string manipulation for the latter.