...
Note that Python does not have a direct equivalent to Java's `Map` class, so we use dictionaries (`dict`) instead. Also, the `ToolIconURL` class is not directly translatable into Python because it seems to be specific to the docking framework used in GHIDRA.