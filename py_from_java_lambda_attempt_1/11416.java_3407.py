...
Note that I've used type hints for the method parameters and variables where possible. This is not required by Python itself but can be helpful in catching errors before runtime.

Also note that some methods are marked as abstract (`compute_hash`), which means they must be implemented in any subclass of `AssemblyResolution`.