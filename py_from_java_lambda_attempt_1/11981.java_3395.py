...
Note that I've used Python's built-in `int` type for the buffer IDs, and assumed that the `get_buffer`, `put_byte`, and `get_bytes` methods of the adapter class take an integer ID as input.