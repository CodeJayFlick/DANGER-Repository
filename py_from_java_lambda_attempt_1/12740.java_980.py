...
Note that Python does not have direct equivalent of Java's `extends` keyword. The class is defined as a standalone class in Python, and the methods are implemented using inheritance from an abstract base class (ABC) or by directly calling the parent class' method (`super().method()`).