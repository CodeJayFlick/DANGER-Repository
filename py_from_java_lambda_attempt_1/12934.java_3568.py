...
Please note that Python does not have a direct equivalent to Java's `BigInteger` class. The code above uses the built-in integer type (`int`) and converts hexadecimal strings into integers using the `int()` function with base 16 as an argument.