...
Note that Python does not have direct equivalents for Java's `int` and `long`, so I used the built-in integer type. Also, Python's floating-point operations are different from those in Java, but this code should still work correctly.