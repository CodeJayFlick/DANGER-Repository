...
Please note that Python does not have a direct equivalent to Java's `Hashtable` or `Collection`. In this translation, I used dictionaries (`{}`) for the points in each quadtree node. For querying all points within a certain range, I used generator expressions and the `yield from` statement (available since Python 3.3).