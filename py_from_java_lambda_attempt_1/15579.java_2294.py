...
Note that Python does not have direct equivalents for Java's `enum` and `interface`, so I've replaced them with regular classes. Also, the equivalent of Java's `@Override` annotation is to simply use the same method name in your subclass (as shown in the `close()` method).