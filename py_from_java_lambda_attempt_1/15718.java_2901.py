...
Note that Python does not have a direct equivalent to Java's `float.NaN` or `Float.isNaN()`. Instead, we use the special float value `float('nan')`, and check if it is equal to itself (which will always be False) in order to detect NaN values.