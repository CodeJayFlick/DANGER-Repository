...
Please note that the translation is not a direct one-to-one conversion. Python and Java are different languages with their own syntax and semantics, so some changes were necessary to make the code work correctly in Python.

Also, please be aware that this code uses libraries like `PIL` (Python Imaging Library) which might need to be installed separately if you don't have them already.