...
class ReadWriteIOUtils:
    ...
    def write_int(output_stream: io.BytesIO, int_value: int) -> None:
        ...
        pass
```
Note that I had to make some assumptions about the missing parts of the code (e.g. `ReadWriteIOUtils.read_long`, etc.). You will need to fill in those implementations based on your specific requirements.

Also, this is not a direct translation from Java to Python, but rather an equivalent implementation in Python. The logic and structure are similar, but some details may have changed due to the differences between the two languages.