...
class TsFileDecodingException(Exception):
    pass
```
Note that I've kept the same variable names and structure as the original Java code, but translated them to Python syntax. Some methods (e.g., `read_length_and_bit_width`, `read_next`, and `has_next_package`) are currently implemented with a `pass` statement, indicating that they need to be filled in according to your specific requirements.

Also, I've used type hints for the method parameters and return types where possible, but Python is dynamically typed so you can ignore these if needed.