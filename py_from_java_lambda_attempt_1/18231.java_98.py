Here is the translation of the Java interface `IMetadataQuerier` to a Python class:
```python
import io
...
class IMetadataQuerier:
    ...
    def clear(self) -> None:
        ...
        pass
```
Note that I've replaced the Java-specific types (e.g., `IChunkMetadata`, `TsFileMetadata`) with more general Python types (e.g., `dict`). You'll need to fill in the implementation details for each method.