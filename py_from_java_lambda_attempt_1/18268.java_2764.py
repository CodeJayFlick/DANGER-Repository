...
* Renamed `TSDataType` to simply `data_types`, as it's not a Java-specific type.
* Replaced `IOException` with Python's built-in exception handling (try/except blocks).
* Changed method signatures to conform to Python's conventions (e.g., removed `throws IOException` from method declarations).
* Simplified the code by removing unnecessary comments and reformatting for readability.