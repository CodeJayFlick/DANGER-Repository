...
Please note that Python does not have a direct equivalent to Java's `ThreadLocalRandom`. Instead, we use the built-in `random` module. Also, I used the `datetime` and `dateutil` modules for date manipulation, which is more natural in Python than creating a new calendar object as you did in your original code.