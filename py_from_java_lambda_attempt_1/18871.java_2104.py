...
Note that Python does not have a direct equivalent to Java's `@Test` annotation. Instead, we define the tests as separate functions and call them at the end of our script if it's being run directly (i.e., not imported as a module).