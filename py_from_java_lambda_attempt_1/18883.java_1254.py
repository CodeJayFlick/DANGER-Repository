...
Note: This code is a direct translation of the Java code and may not work as expected. The `IcebergTable` class, for example, seems to be custom-made in Java but does not have an equivalent in Python. You would need to create your own implementation or use a library that provides similar functionality.

Also note that this code uses the `requests` library to make HTTP requests and parse JSON responses.