...
- The `Bukkit` class and its methods are equivalent to their Java counterparts.
- The `OfflinePlayer` class is a subclass of Python's built-in `object`.
- The `ChangeMode` enum has values like `ADD`, `REMOVE`, `SET`, and `RESET`.

Also, note that this translation does not include the static block from the original code.