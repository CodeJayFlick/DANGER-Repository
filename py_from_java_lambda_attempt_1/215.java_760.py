...
Note that this translation is not a direct conversion from Java to Python, but rather an equivalent implementation in Python. The `async` and `await` keywords are used for asynchronous programming, which does not exist in the original Java code.