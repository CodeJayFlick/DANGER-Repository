...
Please note that this translation is based on the provided Java code and might not be a direct equivalent of the original code. The `ProgramContextImpl` class has been left as it was in the Java code since its implementation seems to be specific to the context where the code is being used, which may vary depending on the Python environment or framework you are using.