...
Please note that Python does not have direct equivalents for Java's `IntPropertyMap`, `LongPropertyMap`, etc. classes, so I replaced them with dictionaries or strings where applicable. Also, the equivalent of Java's `Iterator` is Python's built-in iterator (`iter()` function).