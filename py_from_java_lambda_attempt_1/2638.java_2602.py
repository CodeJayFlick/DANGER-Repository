...
This Python translation maintains the same structure and functionality as the original Java code. Note that some minor adjustments were made to accommodate differences in syntax between the two languages, such as using `iter` instead of `Iterator`, `next` instead of `hasNext() && next()`, etc.