...
Note that the above Python code is not a direct translation of the given Java code. It's more like an equivalent implementation in Python with some differences due to language-specific features and best practices.

The `CompletableFuture` class used in the original Java code does not have a direct equivalent in Python, so I replaced it with simple asynchronous functions that return futures using the built-in `asyncio` library.