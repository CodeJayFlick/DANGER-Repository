...
```
Note that I've used the `map` function to convert each byte in the array to a character (since Python strings are Unicode), and then joined them together with an empty string (`''`). The `int` function is used to parse the resulting string as an integer.