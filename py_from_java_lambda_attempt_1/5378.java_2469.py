...
```
    ...
    def process_symbols(self, program):
        if not isinstance(program, Program):
            raise TypeError("Invalid type for 'program' - it should be a Program")
        if not isinstance(program, Program):
            raise TypeError("Invalid type for 'program' - it should be a Program")

    def load(self, provider):
        if not isinstance(provider, ByteProvider):
            raise TypeError("Invalid type for 'provider' - it should be an instance of ByteProvider"):
```

    def process_symbols(self, program):

    def load(self, provider):  if not isinstance(program, ByteProvider):
            raise TypeError("Invalid type for 'program' - it should be a Program"
        if not isinstance(provider, ByteProvider):
            raise TypeError("Invalid type for 'provider' - it should be an instance of CoffFileBytes
```
