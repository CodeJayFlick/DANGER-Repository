...
Note that Python does not have direct equivalents for Java's `Map.Entry` and `FileSystemRefManager`, so I've used dictionaries (`defaultdict`) to represent the cache, and replaced these classes with simple dictionary lookups.