...
Please note that Python does not have direct equivalent of Java's `AbstractMsf`, `PdbReaderOptions` and other classes. You would need to define these in your own code or use existing libraries if they exist.

Also, the `match` statement is a new feature introduced in Python 3.10. If you are using an earlier version, you can replace it with a traditional `if-elif-else` block.