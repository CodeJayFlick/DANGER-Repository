...
Note: In Python, there's no direct equivalent to Java's `@Override` annotation. The method name and signature must match exactly for the method to be considered an override.

Also note that I've used type hints (`'Function'`) in the function parameters, but these are not enforced by Python itself; they're just a way of documenting what types you expect each parameter to have.