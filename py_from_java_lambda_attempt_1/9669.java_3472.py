...
class UniformViewToIndexMapper:
    ...
    def index_model_data_changed(self):
        pass  # no-op
```
Note that I've used Python's built-in `int` and `str` types, as well as the `BigInteger` class from the `gmpy2` library (which is not included in this translation). You may need to install `gmpy2` separately if you want to use it.