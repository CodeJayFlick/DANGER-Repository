...
Note that Python does not have a direct equivalent to Java's `synchronized` keyword. Instead, we use the `Lock` class from the `threading` module to ensure thread safety in our methods.