...
This Python code is equivalent to the Java code you provided. It includes test cases for basic loading, slow node showing progress bar, and in-progress nodes. The `GTreeSlowLoadingNode` class represents a tree with slow-loading nodes that can be cancelled by pressing a button.

Note: This translation assumes that some classes (like JFrame) are equivalent to their Java counterparts, but they might not work exactly the same way due to differences between languages.