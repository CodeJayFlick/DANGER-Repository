...
Note that Python does not have direct equivalents of Java's `Class`, `Method` and `Field`. Instead, we use the built-in functions like `type()` to get the class type, `inspect.getmembers()` to iterate over methods and fields. We also don't need explicit try-catch blocks as exceptions are propagated up in Python.