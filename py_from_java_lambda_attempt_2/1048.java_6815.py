...
Please note that Python does not have direct equivalent of Java's `Map` and `ImmutableMap`. I used the built-in dictionary (`dict`) for simplicity. Also, there is no exact equivalent to Java's `WeakValueHashMap`, so I used a simple weak reference dictionary from the standard library (`weakref`).