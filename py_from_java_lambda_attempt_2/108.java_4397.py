...
Note that Python does not have an equivalent to Java's enum, so we're using a class instead. Also, the `Color` array in Java is replaced with a dictionary (`colors`) where each key corresponds to a specific color value.