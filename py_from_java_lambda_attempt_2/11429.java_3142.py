...
Note that Python does not have direct equivalent of Java's `regionMatches` method, so I used string slicing to achieve the same result. Also, in Python, we don't need to explicitly specify types for variables and function parameters like we do in Java.