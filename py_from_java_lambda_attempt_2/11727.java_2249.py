...
class Pattern:
    ...
    def restore_xml(self, el: ET.Element):
        raise NotImplementedError("Must be implemented by subclass")
```
Note that I used the `raise NotImplementedError` statement to indicate which methods must be implemented by subclasses. This is equivalent to the abstract method declarations in Java.

Also, I used Python's built-in `int`, `bool`, and `object` types instead of importing specific libraries like `java.io.PrintStream`.