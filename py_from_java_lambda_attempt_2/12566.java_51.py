...
Note that I've used type hints for the function parameters and return types. This is a Python feature called "type hinting" which allows you to specify the expected types of variables or function parameters. It's not enforced at runtime, but it can be useful for static analysis tools like mypy.

I have also assumed some classes (like `Program`, `DataTypeManager`) that are missing in your code and replaced them with placeholder classes (`class Program: pass` etc.).