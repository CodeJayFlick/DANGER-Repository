...
```
Note that I used the `peek_next_byte()` method to read a single byte from the reader, and then compared it to various constants. In Python, we don't have an exact equivalent of Java's switch statement, so I used if-elif statements instead.