...
Note that Python does not have direct equivalents of Java's `Set`, `Map`, and other data structures. Instead, you can use built-in types like lists (`[]`) or dictionaries (`{}`) to achieve similar functionality.

Also note that the translation is not always a 1:1 mapping from Java code to equivalent Python code. Some parts may be rewritten in different ways due to differences between languages.