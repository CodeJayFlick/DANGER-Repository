...
Please note that the `convertInputStreamToByteArray` method is not directly equivalent to its Java counterpart. The Python code reads from the input stream in chunks and writes them to a byte array, whereas the Java code does this by reading into an intermediate buffer before writing it to another buffer.