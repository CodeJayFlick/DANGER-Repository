...
Note: Python does not have a direct equivalent to Java's `@Test` annotation. Instead, we use the built-in `unittest` module and define test cases as methods of a class that inherits from `unittest.TestCase`.