...
Note that Python does not have direct equivalent of Java's `@Setter` and `@Getter`, so I used the property decorator to achieve similar functionality. Also, Python is dynamically typed language, so there is no need for explicit type casting like in Java.