...
Note that Python does not have a direct equivalent to Java's `HashSet` or `StringBuilder`. Instead, we use the built-in set data structure and string concatenation. The `isHyphenLike` method is translated into a simple membership test using the `in` operator.