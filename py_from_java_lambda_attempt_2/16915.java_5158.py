Here is the translation of the Java interface `IUserManager` into a Python class:
...
- In Python, we don't have an exact equivalent to Java's `interface`. Instead, we use abstract base classes (ABCs) and interfaces. However, since the original code seems more like a Java interface than a class, I've kept it as is.

- The methods in this class are currently raising `NotImplementedError` exceptions because they're not implemented yet. You would need to implement these methods according to your specific requirements.

- Some of the method names have been slightly modified for better readability and Pythonic naming conventions.