...
```
Note that I've replaced the Java-specific constructs (e.g., `@Override`, `throws` clauses) with Python's equivalent syntax. Additionally, I've defined two helper functions (`has_space` and `get_occupied_space`) as placeholders, since their implementation is specific to your use case.

Also, I've used Python 3.x style type hints for the function return types (e.g., `-> int`). If you're using an earlier version of Python, feel free to remove these.