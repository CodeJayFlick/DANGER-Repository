Here is the translation of the Java interface `Trigger` into a Python class:
...
Note that Python does not have built-in support for generics like Java. The `fire_scalar_value`, `fire_float_value`, etc., methods are equivalent to the overloads of the same name in the original Java code, but they do not enforce type safety at runtime.