...
Note that Python does not have direct equivalents for Java's `DataOutputStream` and `ByteBuffer`. Instead, we use the built-in file I/O functions or libraries like `struct` to serialize and deserialize data.