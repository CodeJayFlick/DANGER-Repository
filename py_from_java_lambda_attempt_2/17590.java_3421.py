...
def write_double(self, output_stream: io.BytesIO) -> int:
    pass
```
Note that I had to make some assumptions about the missing parts of the code (e.g. `put_double`, `write_time`, and `write_double` methods). These would need to be implemented in Python based on their Java counterparts.

Also, this translation is not a direct copy-paste from Java to Python, but rather an interpretation of the original code into Python syntax.