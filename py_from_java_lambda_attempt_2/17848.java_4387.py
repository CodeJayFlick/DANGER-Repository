...
```
Note that I did not translate the `@BeforeClass` and `@AfterClass` annotations, as they are specific to Java's JUnit testing framework. In Python, you would typically use a test runner like Pytest or Unittest to manage your tests.

Also, I replaced the `Assert.assertEquals()` method with simple printing statements, since there is no direct equivalent in Python. If you want to compare expected and actual results, you can use Python's built-in `assert` statement or a testing library like Hypothesis.