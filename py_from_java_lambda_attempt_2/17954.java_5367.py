...
class LRUCache:
    ...
    def remove_first_occurrence(self, target_index):
        ...
        return False
```
Note that I used a few Python-specific constructs:

* Lists are used instead of arrays.
* The `__init__` method is used to initialize the object's state.
* The `get`, `set`, and other methods use self-reference (`self`) to access the object's attributes.
* The `len` function is used to implement the length property.
* The `max` function with a lambda function as the key is used to find the last index in the list.

Also, I replaced the Java-specific code (e.g., `Integer.MIN_VALUE`, `Arrays.fill`) with Python equivalents.