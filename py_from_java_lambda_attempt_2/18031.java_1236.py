...
1. The `TSDataType` enum and its values are defined elsewhere in your code.
2. The `IoTDBConnectionException`, `StatementExecutionException`, and other exceptions are also defined elsewhere in your code.

Also, please note that Python does not have direct equivalents to Java's `byte[]` arrays or `ByteBuffer`. Instead, you can use the built-in `bytes` type for storing binary data.