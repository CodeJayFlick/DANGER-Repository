...
Please note that Python does not have a direct equivalent to Java's `ThreadLocalRandom`. Instead, we use the built-in `random` module. Also, I used the `date` and `timedelta` classes from the datetime module for generating random dates of birth.