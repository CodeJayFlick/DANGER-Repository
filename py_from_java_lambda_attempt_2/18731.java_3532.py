...
Note that Python does not have a direct equivalent to Java's `ThreadLocalRandom`, so I used the built-in `random` module instead. Also, Python's datetime library is more straightforward than Java's Calendar class, and it allows for easier date manipulation.