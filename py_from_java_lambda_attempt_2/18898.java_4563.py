...
- The `QuarkusNessieAuthorizationConfig` class is equivalent to a Python dictionary or object with an "enabled" attribute.
- The `CompiledAuthorizationRules` class is equivalent to a list of functions (or objects with execute methods) in Python.
- The `AccessControlException` exception is equivalent to the built-in `Exception` type in Python, but you may want to create your own custom exception class if needed.