...
if __name__ == "__main__":
    asyncio.run(main())
```
Note that I've used Python's `async` and `await` keywords to translate the Java code, which is a common pattern in Python for handling asynchronous operations. Additionally, I've defined custom classes such as `ConfigApiImpl`, `NessieConfiguration`, and `ServerConfig` based on their respective counterparts in the original Java code.

Please note that this translation may not be exact, as some details like error handling or specific implementation might have been omitted to keep it concise.