...
Note that Python does not have a direct equivalent of Java's `@Nullable` and `@SuppressWarnings("null")`, so I removed those annotations. Also, the `LanguageChangeListener` interface is not directly translatable to Python, as it seems to be related to language changes in SkriptLang.