...
Note that this is not a direct translation of Java to Python. The `JdiManager` class in Python does not have all the same functionality as its counterpart in Java, and some parts are simplified or removed for simplicity.

The `@abstractmethod` decorator is used to define abstract methods, which must be implemented by any subclass of `JdiManager`.