...
Note that Python does not have an exact equivalent to Java's `enum` type. Instead, we use the built-in `enum` module from Python 3.4 onwards or define our own enum-like class using a dictionary.