...
Note that Python does not have direct equivalents of Java's `Map` and `List`, so I used dictionaries (`{}`) to represent maps and sets (e.g., `{GdbDefaultOffer(...)}`) to represent lists.