...
Please note that the above Python code is not a direct translation of the Java code. It's more like an adaptation to fit into Python syntax and structure. Some parts, such as GUI components (like JComponents), may have different implementations in Python compared to Java.

Also, some classes (`GFilterTable`, `SampleTableModel`, `FunctionAlgorithm`) are placeholders for actual Python classes that would be used in the original code.