...
- The `Program` class has methods `get_listing()` and `create_module()`.
- The `Listing` class has method `get_module(tree_name, parent_name)`.
- A `DuplicateNameException` is raised by some other exception in Python.
- You have a way to get an instance of the `Program` class.