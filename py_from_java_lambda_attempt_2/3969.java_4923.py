...
```
Note that I've used Tkinter for creating a simple graphical user interface (GUI) in Python. The `ControlPanel` class is responsible for managing the layout and widgets of the GUI, while the `SelectionScopeWidget` and `SearchDirectionWidget` classes are custom widgets that can be packed into the main window.

Please note that this translation might not cover all aspects of the original Java code, as some parts (like event handling) were omitted.