...
```
Note that I've used Python's `os` module to perform file system operations, and created a few classes (`ResourceFile`, `ScriptInfo`) to represent script files and their metadata. The rest of the code is similar to the original Java implementation, with some minor adjustments for Python syntax and semantics.