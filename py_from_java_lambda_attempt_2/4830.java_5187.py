...
Note that Python does not have a direct equivalent to Java's `HashMap` and `List`. Instead, we use the built-in dictionary (`dict`) for mapping and list comprehension for iterating over keys. Also, Python's type system is dynamically typed, so there are no explicit checks like in Java code.