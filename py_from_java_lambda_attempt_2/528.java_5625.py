...
Note that Python does not have a direct equivalent to Java's `interface` keyword. Instead, we define an abstract base class (`class`) with methods declared but no implementation.

The `VTIndices2` enum is translated directly into a Python `Enum`.