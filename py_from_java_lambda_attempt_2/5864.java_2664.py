...
Note that Python does not have direct support for generics like Java, so the type parameter `T` has been removed. Also, the method `next()` now uses a generator to return each element in turn, rather than returning them all at once as an array.