...
Note that Python's `bytearray` and `bytes` types are used to represent the buffer, which is equivalent to Java's `byte[]`. The `to_bytes()` method is used to convert integers to bytes in big-endian byte order.