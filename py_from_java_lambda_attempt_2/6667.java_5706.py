...
Note that I've used Python's built-in `ValueError` exception instead of Java's `IllegalArgumentException`. Also, the equivalent to Java's `StringBuilder` is Python's string formatting.