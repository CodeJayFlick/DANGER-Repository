...
Note that Python does not have direct equivalent of Java's `struct` and `byte[]`. Instead, we use the built-in string decoding method to convert bytes into strings. Also, Python's file reading is different from Java's, so I used a hypothetical `BinaryReader` class for simplicity.