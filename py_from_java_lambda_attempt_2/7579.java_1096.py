...
Note that Python does not have direct equivalent for Java's `Writer` and `FileWriter`. Instead, we use the built-in file handling functions like `open()` and `'a'` mode to append data to a file.