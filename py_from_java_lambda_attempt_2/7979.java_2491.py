...
Note that I have translated the Java code into Python as closely as possible. However, there are some differences between the two languages that may affect how the code works in practice.

For example:

* In Java, `int` is a 32-bit integer type, while in Python, integers can be arbitrarily large.
* In Java, `long` is a 64-bit integer type, while in Python, long integers are also arbitrary-precision.
* In Java, the `StringBuilder` class is used to build strings incrementally. In Python, you would typically use the `str.join()` method or the `''.join(map(str, ...))'` idiom instead.

I have not translated any of these differences into equivalent code in this translation.