...
class ManagedProcedureSymbolApplier:
    def begin_block(self, address: object, name: str, length: int) -> None:
        ...
        pass
```
Note that I had to make some assumptions about the types and methods of certain classes (e.g. `AbstractBlockMsSymbol`, `FunctionSymbolApplier`, etc.) since they were not provided in the original Java code. Additionally, Python does not have a direct equivalent to Java's checked exceptions, so I removed those parts of the code.