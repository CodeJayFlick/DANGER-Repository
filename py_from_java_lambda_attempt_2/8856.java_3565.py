...
* Attribute access using `self.` syntax
* Method definitions with `def` keyword
* Property decorators (`@property`)
* Hashing function implementation using built-in `hash()` function
* Equality comparison using `__eq__` method and `is_equal()` helper function from `SystemUtilities`
* String formatting using f-strings

Also, I've assumed that the `SourceType` class is defined elsewhere in your Python code. If it's not, you'll need to define it or replace it with a suitable alternative.