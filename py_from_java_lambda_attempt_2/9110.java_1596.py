...
Please note that you need to fill in the implementation details for some methods, such as `VTTestEnv.create_session`, `VTTestEnv.get_source_program`, `VTTestEnv.get_destination_program` and `VTTestEnv.get_vt_controller`.