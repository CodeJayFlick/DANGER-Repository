...
class FieldLocation:
    pass
```
Note that I had to create a `FieldLocation` class in Python, as there is no equivalent built-in type. You may need to modify this class depending on how you intend to use it.

Also, the Java code uses an `ActionListener` interface and `Timer` class from the Swing library, which do not have direct equivalents in Python. Instead, I used a simple timer loop with a 500ms delay, and implemented the equivalent of the `actionPerformed` method as a separate function (`update_timer`).