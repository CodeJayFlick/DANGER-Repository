...
- The `BigInteger` class in Java has been replaced with Python's built-in integer type (`int`) for simplicity.
- The methods `get_text_for_fields_in_layout`, `is_string_selection`, and `get_field_range` are assumed to be part of a larger class, as they seem to rely on the existence of other classes or variables.