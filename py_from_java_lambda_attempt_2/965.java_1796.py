...
Note that Python does not have direct equivalents to Java's `Pointer` and `COMUtils`, so I've replaced them with equivalent constructs. Also, the `try-preferred-interfaces` logic is implemented using try-finally blocks in Python.