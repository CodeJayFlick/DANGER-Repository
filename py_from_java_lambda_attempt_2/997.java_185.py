...
Please note that Python does not have direct equivalent of Java's `Map` and `ImmutableMap`. I used a dictionary (`dict`) to represent the map. Also, there is no direct equivalent of Java's `Pointer`, so it was removed from the code.